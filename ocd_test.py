...
ocd = ocd.OCD10(num_classes= 196).to(device)
...
test = torch.rand(1, 3, 180, 180).to(device)
...
test_set = datasets.ImageFolder(root='stanford-car/car_data/train/', transform = transform_test)

train_loader = torch.utils.data.DataLoader(dataset = train_set, shuffle = True, drop_last= True, batch_size = 16, num_workers= 4)
test_loader = torch.utils.data.DataLoader(dataset = test_set, shuffle = True, drop_last= True, batch_size = 16, num_workers= 4)
...
training_epochs = 100
learning_rate_ocd = 0.5
learning_rate_vgg = 0.03
learning_rate_resnet = 0.1



optimizer_ocd = optim.SGD(ocd.parameters(), momentum =0.9, lr = learning_rate_ocd)
...
resnet_acc_plt = vis.line(Y = torch.Tensor(1).zero_(), opts = dict(title =
...
def acc_check(net, name, test_loader, epoch, save = 1):
    ...
    if epoch % 5 == 0:
        if save:
            torch.save(net.state_dict(), "./models/stanford/{}_model_epoch_{}_acc_{}.pth".format(name, epoch, acc))
